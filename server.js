  ...
  try {
    ...
    for (const col of colaboradores) {
      try {
        ...
        if (!email) {
          console.log(`[aviso] matrícula ${col.matricula} sem email cadastrado`)
        ...
        const doc = await criarDocumento({ nome: `${nomeDoc} - ${nomeReal}`, pdfBuf, pdfNome: `${col.matricula}.pdf`, email, nomeColaborador: nomeReal, mensagem })
...
async function criarDocumento({ nome, pdfBuf, pdfNome, email, nomeColaborador, mensagem }) {
  ...
  const res  = await fetch('https://api.autentique.com.br/v2/graphql', {
    method: 'POST', headers: { Authorization: `Bearer ${AUTENTIQUE_TOKEN}`, ...form.getHeaders() }, body: form,
  })
