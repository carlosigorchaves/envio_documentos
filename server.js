...
app.use((req, res, next) => {
  ...
  if (OCR_SECRET && secret !== OCR_SECRET) {
...
// Rota principal: recebe PDF, faz OCR, separa e envia para Autentique
app.post('/processar', upload.single('pdf'), async (req, res) => {
  ...
  try {
    ...
    if (!totalPaginas) {
      return res.status(400).json({ erro: 'Não foi possível ler o PDF.' })
    }

    console.log(`[ocr] processando ${totalPaginas} páginas...`)

    // Fase 1: OCR em todas as páginas
    const mapa = {}
    ...
    for (let pg = 1; pg <= totalPaginas; pg++) {
      ...
      try {
        ...
        if (output && output !== 'NAO_IDENTIFICADO' && output.includes('||')) {
          ...
          if (!mapa[chave]) {
            mapa[chave] = { matricula: matricula.trim(), nome: nome.trim(), paginas: [] }
          }
        ...
        console.error(`[ocr] erro página ${pg}:`, err.message)
      }
    ...
    if (!colaboradores.length) {
      return res.status(400).json({ erro: 'Nenhum colaborador identificado pelo OCR.' })
    }
    ...
    for (const col of colaboradores) {
      try {
        // Separa páginas em PDF individual
        ...
        // Busca email no Supabase pela matrícula
        const { data: encontrados } = await sb()
          .from('colaboradores')
          .select('id, email, nome')
          .or(`cpf.eq.${col.matricula},extras->>matricula.eq.${col.matricula}`)
          .eq('status', 'pendente')
          .limit(1)

        const email    = encontrados?.[0]?.email || null
        const nomeReal = encontrados?.[0]?.nome   || col.nome

        if (!email) {
          console.log(`[envio] matrícula ${col.matricula} sem email — pulando`)
          resultados.push({ matricula: col.matricula, nome: col.nome, ok: false, erro: 'Email não encontrado' })
          continue
        }
        ...
        const { data: inserted } = await sb().from('colaboradores').insert({
          ...
          cpf:      col.matricula,
          status:   'pendente',
          extras:   { matricula: col.matricula, paginas: col.paginas },
        }).select().single()
        ...
        const doc = await criarDocumentoAutentique({
          ...
          pdfBuf,
          pdfNome:  `${col.matricula}_${nomeReal}.pdf`,
          email,
    ...
    res.json({
      ...
      enviados,
      semEmail: resultados.filter(r => !r.ok && r.erro === 'Email não encontrado').length,
      erros:    resultados.filter(r => !r.ok && r.erro !== 'Email não encontrado').length,
      sandbox:  SANDBOX,
      resultados,
    })
...
async function criarDocumentoAutentique({ nome, pdfBuf, pdfNome, email, nomeColaborador, mensagem }) {
  const query = `
...
app.listen(PORT, () => {
  console.log(`OCR Service rodando na porta ${PORT}`)
