...
export async function atualizarStatus(documentId, { email, status, quando, arquivoUrl } = {}) {
  let q = sb().from('colaboradores')
    .select('id,status,visualizado_em,assinado_em,rejeitado_em,arquivo_assinado_url')
  ...
  for (const row of rows) {
    ...
    if (Object.keys(up).length) { await sb().from('colaboradores').update(up).eq('id', row.id); alterou = true }
  }
  return alterou
}
